...
Using **fabricatr** begins by calling the function [`fabricate()`](../reference/fabricate.html). [`fabricate()`](../reference/fabricate.html) can be used to create single-level of hierarchical data. There are three main ways to call [`fabricate()`](../reference/fabricate.html): 
...
country_data <-
  fabricate(
    ...
    provinces = add_level(N = 10,
                      has_nat_resources = draw_binary(probs=0.3, N=N),
                      has_manufacturing = draw_binary(probs=0.7, N=N))
  )
head(country_data)
```
```{r echo=FALSE}
...
Finally, the province-level variables are created using the [`draw_binary()`](../reference/draw_inary.html) function. This is a function provided by **fabricatr** to make simulating discrete random variables simple. When you simulate your own data, you can use **fabricatr**'s functions, R's built-ins, or any custom functions you wish. [`draw_binary()`](../reference/draw_binary.html) is explained in [our tutorial on variable generation using **fabricatr**](variable_generation.html)
...
Again, we see the use of [`draw_binary()`](../reference/draw_binary.html). Using this function is covered in our tutorial on [generating discrete random variables](variable_generation.html), linked below.

# Next Steps

You've seen **fabricatr**'s ability to generate single-level and hierarchical data, which is enough to get you started on using the package. From here, you can learn about [generating discrete random variables](variable_generation.html), [using **fabricatr** to bootstrap or resample hierarchical data](resampling.html), [using **fabricatr** to generate cross-classified data](cross_classified.html), or [advanced features](advanced_features.html).
