...
Note that `cross_levels()` takes a single required argument, which is of the form `by = join_by(...)`. This join command tells **fabricatr** how to assemble your data. In this case, we are telling it to join the countries data frame to the years data frame, resulting in country-year observations.
...
schools_data <- fabricate(
  ...
  students = link_levels(N = 1500, by = join_by(primary_schools, secondary_schools))
)
```
...
schools_data <- fabricate(
  ...
  students = link_levels(
    N = 1500, by = join_by(primary_schools, secondary_schools),
...
corr_data <- fabricate(
  ...
  secondary_schools = add_level(N = 15, ss_quality = runif(N, 1, 10), nest = FALSE),
  students = link_levels(
    N = 1500, by = join_by(ps_quality, ss_quality, rho = 0.5),
    SAT_score = 800 + 13 * ps_quality + 26 * ss_quality +
      rnorm(N, 0, 50)
  )
)
