...
```
```{r echo=FALSE}
knitr::kable(head(simulated_quake_data), format.args=list(big.mark = ","))
```
...
country_data <-
  fabricate(
    ...
    provinces = add_level(N = 10,
                      has_nat_resources = draw_binary(prob = 0.3, N = N),
                      has_manufacturing = draw_binary(prob = 0.7, N = N))
  )
head(country_data)
```
```{r echo=FALSE}
...
new_citizen_data <-
  fabricate(
    data = citizen_data,
    countries = modify_level(avg_temp = runif(N, 30, 80)),
    provinces = modify_level(conflict_zone = draw_binary(N, prob = 0.2 + has_nat_resources * 0.3),
                             infant_mortality = runif(N, 0, 10) + conflict_zone * 10 + 
                               (avg_temp > 70) * 10),
    citizens = modify_level(college_degree = draw_binary(N, prob = 0.4 - (0.3 * conflict_zone)))
  )
```                        
