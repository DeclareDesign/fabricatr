...
The simplest possible type of data is a binary random variable (also called a bernoulli random variable). Generating a binary random variable requires only one parameter `probs` which specifies the probability that outcomes drawn from this variable are equal to 1. By default, [`draw_binary()`](../reference/draw_vbinary.html) will generate `N = length(probs)` draws. `N` can also be specified explicitly. Consider these examples:

```{r}
draw_binary_ex = fabricate(N = 3, p = c(0, .5, 1),
                           binary_1 = draw_binary(probs = p),
                           binary_2 = draw_binary(N = 3, probs = 0.5))
```

In addition to binary variables, you can make data from repeated Bernoulli trials ("binomial" data). This requires using the [`draw_binomial()`](../reference/draw_binomial.html) function and specifying an argument `trials`, equal to the number of trials.
...
[`draw_count()`](../reference/draw_count.html) allows you to create Poisson-distributed count outcomes. These require that the user specify the parameter `means`, equal to the Poisson distribution mean (often referred to as `lambda` in statistical formulations of count data).
...
[`draw_categorical()`](../reference/draw_categorical.html) can generate non-ordered, categorical data. Users must provide a vector of probabilities for each category (or a matrix, if each observation should have separate probabilities).
...
In the second example, each unit has the same probability of getting a given category. [`draw_categorical()`](../reference/draw_categorical.html) will issue a warning to remind you that it is interpreting the vector in this way.
```{r}
...
If `sd` is not supplied, each cluster will be assumed to have a within-cluster standard deviation of 1. If `x` is not supplied, each cluster will be assumed to be mean zero. If `ICC` is not supplied, it will be set to 0.5.
