...
check_variables_named <- function(data_arguments, call_type = "add_level") {
  if("" %in% names(data_arguments)) {
    ...
    if(length(data_arguments) > 1 &&
       "" %in% names(data_arguments)[1:(length(data_arguments)-1)]) {
      # There was a variable inside the call that wasn't named.
      stop("All variables inside a ", call_type, " call must be named. The ",
           "supplied arguments were named: ", new_names, ".")
    } else {
      # There was a variable at the end of the call that wasn't named.
      # Hanging comma? Maybe?
      stop("All variables inside a ", call_type, " call must be named. The ",
           "supplied arguments were named: ", new_names, ". A possible cause ",
           "of this error could be an extra comma at the end of the argument ",
           "list.")
    }
  }
}
