...
modify_level = function(N = NULL,
  ...
  if(is.null(ID_label)) {
    stop("You can't modify a level without a known level ID variable. If you are adding nested data, please use add_level")
  }
  ...
  if(length(level_unique_variables) != length(write_variables) &
    ...
    stop("Your modify_level command attempts to generate a new variable at the level \"", ID_label,
  ...
  # Our subset needs these columns -- the level variable, all the unique variables
  # we are going to use to write, and then in case the latter is "", remove that dummy
  # obs.:
